...
<nav id="navbar">
    <div class="container">
        ...
        <div class="page">ORDER TO EXPORT</div>
        <div></div>
    </div>
</nav>
...
<script>
    ...
    getData()
    function getData(){
        fetch('https://andrew02.it/api/getOrders', {
            method: "GET",
        ...
        .then(resp => {
            if (resp.result) {
                ...
                resp.orders.forEach(o=>{
                    document.querySelector("#data tbody").insertAdjacentHTML('beforeend', `
                        <tr>
                            <td>${o.id}</td>
                            <td>
                               ${new Date(o.data_consegna).toLocaleDateString()}
                            </td>
                            <td>
                               ${o.indirizzo}
                            </td>
                            ...
                            <td>
                                <button onclick="exportOrder(${o.id})" class="btn info">Esporta</button>
                            </td>
                        </tr>
                    `)
                })
                if(!resp.orders.length){
                    document.querySelector("#data tbody").insertAdjacentHTML('beforeend', `
                        <tr>
                            <td colspan="5">Nessun ordine da esportare</td>
                        </tr>
                    `)
    ...
    function exportOrder(id){
        fetch('https://andrew02.it/api/export', {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: id, negozio: document.querySelector('td[data-id="'+id+'"] .negozio').value})
        })
            .then(res => res.json())
            .then(data => {
                if (!data.result) {
                    ...
                    getData()
                    alert("Ordine esportato con successo")
                }
                return false;
            })
    }
